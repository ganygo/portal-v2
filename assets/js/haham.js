(function(exports) {
	...
	function build(sayfalar,divId){
		...
		try{
			...
			var s='<form class="px-0"><div class="row">'
	...
	function grid(item,bRoot,insideOfModal){
		...
		if(insideOfModal==false){
			...
			s=`<div id="${item.id}" class="table-responsive p-0 ${item.options.show.infoRow?'mt-1':''}"></div>
			${gridModalTemplate(item)}
			`
		}else{
			s=`<div id="${item.id}" class="table-responsive p-0 ${item.options.show.infoRow?'mt-1':''}">${gridHtml(item,false,true)}</div>`
	...
	function gridButtonOptions(item,bRoot,insideOfModal){
		...
		function gridHtml(item,bRoot,insideOfModal=false){
			...
			if(item.options.show.infoRow){
				//				<a class="btn btn-success btn-sm" href="javascript:gridCSVExport('${btoa2(JSON.stringify(item))}')" title="CSV indir"><i class="far fa-file-excel"></i><i class="ms-2 fas fa-download"></i></a>
				s+=`
				<!-- info row -->
				<div class="border p-1">
				<div class="d-md-flex pt-1 px-1">
		...
		function gridModalTemplate(item){
			...
			var s=`
			<div class="modal" id="modalRow${item.id}" tabindex="-1" role="dialog" data-bs-backdrop="static" data-bs-keyboard="true" aria-labelledby="modalRow${item.id}Label" aria-hidden="true">
...
function card(item){
	// var s=`<div class="card cerceve1 ${item.col || ''} p-0 m-2 mb-2 ${item.visible===false?'hidden':''}">
	var s=`
...
function numberBox(item){
	return group(`<input type="number" class="form-control text-end ${item.class || ''}" id="${item.id}" name="${item.name}" placeholder="${item.placeholder || item.title || item.label}" ${item.required?'required="required"':''} ${item.readonly==true?'readonly':''} onchange="${item.onchange || ''}" autocomplete="off" autofill="off" spellcheck="false" value="${item.value || 0}">`,item)
}
...
function remoteLookup(item){
	...
	if(item.noGroup===true){
		...
		s=`<div class="${item.col || ''} p-1 ${item.visible===false?'hidden':''}">
		<div class="form-group">
		<label class="m-0 p-0 ${item.required?'form-required':''}">${item.title || ''}${helpButton(item)} ${item.lookupTextField?'<span class="ms-3 bold small text-success" id="' + item.id + '-original-text"></span>':''}</label>
		${input}
		</div>
		</div>
		`
